				...
				if (user === null) {
					...
					unzipper.on('extract',function(log){
						...
						files.forEach(function(file){
							if(file.substring(file.lastIndexOf(".")+1,file.length) == "DS_Store"){//"DS_STORE 파일이면 do nothing"
					...
					walker.on('end', function() {
						...
						*/
						...
						// 현재 이코드의 문제점: directory 안에 (cpp코드나 py코드)와 디렉토리가 같이있으면 caught ERROR! 오류를 뿜는다. 왜그런지 잘 모르겠따. - completed
